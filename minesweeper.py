...
class MinesweeperAI():
    ...
    def getNeighbors(self,cell):
        '''
        returns the neighbors of a cell and their mine count
        '''
        i,j=cell
        neighbors=set()
        surronding_rows,surronding_cols=range(i-1,i+2),range(j-1,j+2)
        heightdim,widthdim=range(self.height), range(self.width)

        for row in surronding_rows:
            if row in heightdim:
                for column in surronding_cols:
    ...
    def add_knowledge(self, cell, count):
        ...
        for sen in self.knowledge:
            ...
            mines_cells=list(sen.known_mines())
            for safe in safe_cells:
                self.mark_safe(safe)
            for mine in mines_cells:
                self.mark_mine(mine)
    ...
    def make_random_move(self):
        ...
        """
        """'if self.moves_made not in self.mines or self.safes:
            random.randrange"""
        possible_moves=[]
